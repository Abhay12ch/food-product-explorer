...
import { searchByName, fetchByCategory, fetchProductByBarcode, isBarcode } from "@/lib/api";
...
function sortProducts(products, sortBy) {
  ...
  return [...products].sort((a, b) => {
    switch (sortBy) {
      ...
      case "grade-asc": {
        const ga = gradeOrder[(a.nutrition_grades || a.nutrition_grade_fr || "z").toLowerCase()] ?? 9;
        const gb = gradeOrder[(b.nutrition_grades || b.nutrition_grade_fr || "z").toLowerCase()] ?? 9;
        return ga - gb;
      }
      case "grade-desc": {
        const ga = gradeOrder[(a.nutrition_grades || a.nutrition_grade_fr || "z").toLowerCase()] ?? 9;
    ...
    <>
      ...
      {error && (
        <div className="mt-8 flex items-center justify-between rounded-xl border border-rose-500/20 bg-rose-500/[0.06] px-5 py-4 text-sm text-rose-300">
          ...
          <button onClick={() => fetchProducts(1, false)} className="ml-4 flex-shrink-0 rounded-lg bg-rose-500/20 px-4 py-1.5 text-xs font-medium text-rose-200 transition-colors hover:bg-rose-500/30">Retry</button>
        </div>
      )}
      {initialLoad && loading && (
        <div className="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {Array.from({ length: 12 }).map((_, i) => (
            <div key={i} className="animate-pulse rounded-2xl border border-white/[0.06] bg-white/[0.03]">
      ...
      {!loading && !initialLoad && sorted.length === 0 && !error && (
        <div className="mt-16 flex flex-col items-center gap-4 text-center">
          ...
          <h3 className="text-lg font-semibold text-slate-200">No products found</h3>
          <p className="max-w-sm text-sm text-slate-500">Try adjusting your search terms or removing filters to find what you&apos;re looking for.</p>
        </div>
      )}
      {sorted.length > 0 && (
        <div id="product-grid" className="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {sorted.map((product) => (<ProductCard key={product.code} product={product} />))}
        </div>
      )}
      {sorted.length > 0 && (
        <Pagination onLoadMore={handleLoadMore} loading={loading && !initialLoad} hasMore={hasMore} totalCount={totalCount} currentCount={products.length} />
      )}
    </>
  );
}
